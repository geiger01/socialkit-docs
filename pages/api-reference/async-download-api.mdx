...
Works for **YouTube, TikTok, Instagram, and Facebook**. Videos are downloaded at up to **1080p**, with a maximum file size of **2 GB** per download. Credits are **1 per minute of video** (minimum 1), charged when the job completes.
...
Maximum video quality: `240p`, `360p`, `480p`, `720p`, `1080p`. **1080p is the ceiling for v2** — higher requests are clamped to 1080p.
...
**`max_duration`** <small style={{margin:'0 10px'}}>integer</small> <small style={{margin:'0  10px  0 0'}}>Optional</small><br/>
A hard cap on the video's length, in **seconds**. If the video is longer than this, it is **rejected before downloading** and the job ends as `failed` with **no credit charged**. Useful for capping cost — since credits are 1 per minute of video, this lets you (or your own users) avoid accidentally spending a large number of credits on a very long video. Omit for no limit.

For example, `max_duration: 1800` rejects anything over 30 minutes; the failed job's `error` reads like `Video is 3:01:00 long, exceeding the 30:00 limit (max_duration=1800s). Not downloaded; no credit charged.`
