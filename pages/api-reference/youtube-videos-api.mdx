...
- `publishedAt` (exact date) and `description` are populated for a channel's most recent videos in a single request (no per-video calls needed). Older/paginated items and playlist items return `null`/empty for these two fields.
