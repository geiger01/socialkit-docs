...
```bash copy
GET https://api.socialkit.dev/tiktok/search?access_key=<your-access-key>&query=funny%20cats&sortBy=relevance&datePosted=month
```
...
```json
{
  ...
  "data": {
    ...
    "totalResults": 10,
    "cursor": "16:202603042344138E2A2236A8CB881E2BF2",
    "hasMore": true
  }
}
```
...
Number of search results to return per request (maximum 100). To retrieve more results beyond the limit, use the `cursor` parameter for pagination.
...
Pagination cursor to retrieve the next page of results. Pass the `cursor` value from a previous response to get the next batch. When `hasMore` is `true`, more results are available.

---

**`sortBy`** <small style={{margin:'0 10px'}}>string</small> <small style={{margin:'0  10px  0 0'}}>Optional</small> <small>Defaults to `relevance`</small><br/>
How to sort search results. Available options:
- `relevance` — Most relevant results (default)
- `likes` — Sort by most liked
- `date` — Sort by newest first

---

**`datePosted`** <small style={{margin:'0 10px'}}>string</small> <small style={{margin:'0  10px  0 0'}}>Optional</small> <small>Defaults to all time</small><br/>
Filter results by when the video was posted. Available options:
- `day` — Past 24 hours
- `week` — Past week
- `month` — Past month
...
### Pagination Fields
- **`totalResults`**: Number of results returned in this response
- **`cursor`**: Pagination cursor — pass this value in the next request to get more results
- **`hasMore`**: `true` if more results are available, `false` if you've reached the end
