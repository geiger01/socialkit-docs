...
The same access key works with our [MCP server](/mcp) at `https://mcp.socialkit.dev`. MCP clients (Claude Desktop, Claude Code, Cursor, and others) authenticate over an `Authorization: Bearer <your-access-key>` header — see the [MCP guide](/mcp) for client-specific setup.
